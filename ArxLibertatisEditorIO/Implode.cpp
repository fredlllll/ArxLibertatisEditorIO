...
void ArxIO_pack_alloc(const char* in, size_t inSize, char**out, size_t* outSize) {

	pkstream strm;

	strm.pInBuffer = (const unsigned char*)in;
	strm.nInSize = inSize;
	...
	//TODO: wtf what does this do?? just copy input to output??
	//memcpy(outBuf, in, inSize);
...
void ArxIO_pack_free(char* buffer) { // NOLINT const would not be appropriate here
	delete[] buffer;
}
